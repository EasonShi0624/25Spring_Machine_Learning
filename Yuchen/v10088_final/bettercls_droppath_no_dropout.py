...
def train_vae(model, train_loader, val_loader, optimizer, device, num_epochs=1, patience=50, alpha=0.1):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        ckpt_dir="/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v10088_final/checkpoint"
        torch.save(model.state_dict(), f"{ckpt_dir}/better_cls_droppath_no_dropout_epoch{epoch}.pt")
        print(f"Saved checkpoint_epoch{epoch}.pt")
