...
IMAGENET_MEAN = torch.tensor([0.485, 0.456, 0.406])  # RGB
IMAGENET_STD  = torch.tensor([0.229, 0.224, 0.225])

def imgnet_norm(x):
    """x in 0‒1, returns (x-mean)/std"""
    mean = IMAGENET_MEAN[None, :, None, None].to(x.device)
    std  = IMAGENET_STD [None, :, None, None].to(x.device)
    return (x - mean) / std

def imgnet_denorm(x):
    """inverse of imgnet_norm, assumes x in normalised space"""
    mean = IMAGENET_MEAN[None, :, None, None].to(x.device)
    std  = IMAGENET_STD [None, :, None, None].to(x.device)
...
class ResNet50Stem(nn.Module):
    """Outputs a 256×32×32 feature map for 128×128 input."""
    def __init__(self, trainable=False):
        ...
        # conv1 (stride 2, 7×7) → 64×64
        # maxpool (stride 2)     → 32×32
        # layer1 (stride 1)      → 256×32×32
        self.stem = nn.Sequential(
            resnet.conv1, resnet.bn1, resnet.relu,
            resnet.maxpool,
            resnet.layer1         # 256 channels, same spatial size
        )
...
class ConvAE(nn.Module):
    def __init__(self, input_channels=3, latent_channels=8, num_classes=170,
        ...
        # ---------- Encoder ----------
        self.resnet_stem = ResNet50Stem(trainable=resnet_trainable)   # NEW
        self.encoder_tail = nn.Sequential(                            # NEW
            SelfAttention(256),
            ResBlock(256)
        )
    ...
    def encode(self, x):
        x = imgnet_norm(x)                        # NEW
        h = self.resnet_stem(x)
        h = self.encoder_tail(h)
        ...
        if self.training:
            ...
            return mean, mean, logvar            # z=mean in eval

    def decode(self, z):
        h = self.post_bottleneck_conv(z)
        out = self.decoder(h)                    # tanh, in -1…1
        return imgnet_denorm(out)                # map back to 0…1
...
def train_vae(model, train_loader, val_loader, optimizer, device, num_epochs=1, patience=50, alpha=0.1):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        ckpt_dir="/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v10088_final/resnet50/checkpoint"
        torch.save(model.state_dict(), f"{ckpt_dir}/resnet50_epoch{epoch}.pt")
        print(f"Saved checkpoint_epoch{epoch}.pt")
