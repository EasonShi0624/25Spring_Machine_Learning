...
seed = 3407                       # pick any integer you like
torch.manual_seed(seed)         # seeds *all* PyTorch devices (CPU + CUDA):contentReference[oaicite:0]{index=0}
torch.cuda.manual_seed_all(seed)   # good habit for multi-GPU jobs
...
class ConvAE(nn.Module):
    def __init__(self, input_channels=3, latent_channels=8, num_classes=170):
        ...
        '''
        self.classifier = nn.Sequential(
            nn.Conv2d(latent_channels, 128, 1),         # 8→128
            nn.Mish(),
...
def train_vae(model, train_loader, val_loader, optimizer, device, num_epochs=1, patience=50, alpha=0.1):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        ckpt_dir="/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v10088_final/more_layer_cls/manual_search/weight1_dropath0.2/checkpoint"
        torch.save(model.state_dict(), f"{ckpt_dir}/seed3407_epoch{epoch}.pt")
        print(f"Saved checkpoint_epoch{epoch}.pt")
