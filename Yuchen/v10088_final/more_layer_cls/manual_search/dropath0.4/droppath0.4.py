...
class SelfAttention(nn.Module):
    def __init__(self, in_dim, num_heads=4, drop_path=0.4):
        super().__init__()
...
def train_vae(model, train_loader, val_loader, optimizer, device, num_epochs=1, patience=50, alpha=0.1):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        ckpt_dir="/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v10088_final/more_layer_cls/manual_search/dropout0.3_weight1_dropath0.2/checkpoint"
        torch.save(model.state_dict(), f"{ckpt_dir}/droppath0.4_epoch{epoch}.pt")
        print(f"Saved checkpoint_epoch{epoch}.pt")
