...
# ---------------------------------------------------------------------
import train_v4_SiLU as base                     # << your original script
...
def finetune(model: ConvVAE, loader, val_loader, optimizer, device, epochs: int):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            out = ckpt_dir / f"third_finetune_epoch{epoch}_acc{val_acc:.3f}.pt"
...
if __name__ == "__main__":
    ...
    # ---------- Run ----------
    init_val_acc = evaluate(model, val_loader, device)
    print(f"Initial checkpoint validation accuracy: {init_val_acc:.4f}")
