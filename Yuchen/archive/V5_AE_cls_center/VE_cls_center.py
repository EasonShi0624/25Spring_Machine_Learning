...
def train(model, train_loader, val_loader, optimizer, scheduler, device,
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for i, (x, y) in enumerate(loop):
            ...
            # center loss on flattened features
            feat = z.view(x.size(0), -1)
            cen_loss = center_loss(feat, y)
            loss = recon_loss + alpha * cls_loss + gamma * cen_loss
            loss.backward()
            ...
            running_cen   += cen_loss.item()   * bsz
            running_n     += bsz
            loop.set_postfix({
                'recon': f"{running_recon/running_n:.6f}",
                'cls':   f"{running_cls  /running_n:.6f}",
                'cen':   f"{running_cen  /running_n:.6f}"
            })
        # validation
        val_recon, val_acc = validate(model, val_loader, device)
        print(f" → VAL recon={val_recon:.4f}, acc={val_acc:.4f}")
        # save best model
        ckpt_dir = "/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/V5_AE_cls_center/checkpoint"
        ckpt_dict = {
            #"epoch": epoch,
        ...
        torch.save(ckpt_dict, f"{ckpt_dir}/round1_checkpoint_epoch{epoch}.pt")
...
if __name__ == "__main__":
    ...
    train(
        ...
        num_epochs=num_epochs, alpha=alpha, gamma= gamma
    )