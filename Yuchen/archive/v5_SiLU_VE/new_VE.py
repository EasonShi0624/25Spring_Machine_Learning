...
def validate(model, loader, device, alpha, gamma, center_loss, vgg, imagenet_mean, imagenet_std, delta):
    model.eval()
    stats = {'recon': 0, 'class': 0, 'center': 0, 'perc': 0, 'n': 0}
    with torch.no_grad():
        for x, y in loader:
            ...
            stats['n'] += bsz
    return {k: stats[k] / stats['n'] for k in ['recon', 'class', 'center', 'perc']}

# ----------------------------
# 5. Training Loop with SGDR
# ----------------------------
def train_ae(model, train_loader, val_loader, optimizer, device,
    ...
    for epoch in range(start_epoch, num_epochs + start_epoch):
        ...
        for x, y in loop:
            ...
            loss = r_loss + alpha * cls_loss + gamma * cen_loss + delta * perc_loss
            loss.backward()
        ...
        val_stats = validate(model, val_loader, device, alpha, gamma, center_loss,
        ...
        ckpt_dir = "/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v5_SiLU_VE/checkpoint"
        ckpt_dict = {
            'epoch': epoch,
        ...
        torch.save(ckpt_dict, f"{ckpt_dir}/checkpoint_epoch{epoch}.pt")
