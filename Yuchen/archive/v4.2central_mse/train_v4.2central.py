...
def weighted_mse(x, x_recon, weight=1.5):
    """
...
def vae_loss(x, x_recon, mean, logvar, kl_weight):
    recon = weighted_mse(x, x_recon, weight=1.5)
...
def train_vae(model, train_loader, val_loader, optimizer, device,
    ...
    for epoch in range(1, num_epochs+1):
        ...
        ckpt_name = f"/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v4.2central_mse/new_weights/checkpoint_beta0.2_epoch{epoch}.pt"
...
if __name__ == "__main__":
    ...
    train_vae(
        ...
        num_epochs=1000,
        beta_max=5e-4,     # max KL weight
        alpha=0.01,        # classification weight
        gamma=0.0001,        # center loss weight
        vgg=vgg,
        imagenet_mean=imagenet_mean,
        imagenet_std=imagenet_std,
        delta= 5 # perceptual loss weight
    )
