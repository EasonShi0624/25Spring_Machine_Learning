...
"""
python smaller_train_v4_SiLU.py --resume /gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v5_SiLU_VE/model_weights_only_907.pt
"""
...
class ConvVAE(nn.Module):
    def __init__(self, input_channels=3, latent_channels=8, num_classes=170):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv2d(128, 256, 3, 1, 1), nn.BatchNorm2d(256), nn.SiLU(), nn.Dropout2d(0.2),
...
def train_vae(model, train_loader, val_loader, optimizer, device,
              ...
              vgg=None, imagenet_mean=None, imagenet_std=None, delta=0.01,resume=args.resume):
    model.to(device)
    ...
    if resume:
            ...
            print("Traing resumed successfully")
            #model.load_state_dict(state_dict)
            #optimizer.load_state_dict(state_dict["optimizer_state_dict"])
            #scheduler.load_state_dict(state_dict["scheduler_state_dict"])
            #start_epoch = state_dict["epoch"] + 1
            #print(f"Resumed from epoch {state_dict['epoch']}.")
            #ckpt = torch.load(resume, map_location=device)
            # if you saved a dict, unpack it:


    for epoch in range(start_epoch, num_epochs + start_epoch):
        model.train()
        train_r_sum, train_k_sum = 0.0, 0.0
        train_cls_sum, train_cen_sum = 0.0, 0.0
        train_perc_sum, train_tot_sum = 0.0, 0.0
        batch_count = 0
        kl_weight = min(0.001, epoch / 20 * 0.0001)
        ...
        for x, y in loop:
            ...
            optimizer.step()
            loop.set_postfix({
            'recon':  f"{r_loss.item():.6f}",
            'kl':     f"{k_loss.item():.6f}",
            'cls':    f"{cls_loss.item():.6f}",
            'center': f"{cen_loss.item():.6f}",
            'perc':   f"{perc_loss.item():.6f}",
            'total':  f"{loss.item():.6f}"
            })
            train_r_sum   += r_loss.item()
            train_k_sum   += k_loss.item()
            train_cls_sum += cls_loss.item()
            train_cen_sum += cen_loss.item()
            train_perc_sum+= perc_loss.item()
            train_tot_sum += loss.item()
            batch_count   += 1
        # Step scheduler once per epoch
        scheduler.step()
        current_lr = optimizer.param_groups[0]['lr']
        avg = lambda s: s / batch_count
        print(
            f"Train ep{epoch}: "
            f"recon={avg(train_r_sum):.4f}  "
            f"kl={avg(train_k_sum):.4f}  "
            f"cls={avg(train_cls_sum):.4f}  "
            f"center={avg(train_cen_sum):.4f}  "
            f"perc={avg(train_perc_sum):.4f}  "
            f"total={avg(train_tot_sum):.4f}"
        )
        ...
        ckpt_dir = "/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v4.1_SiLU_continue/smaller_lr_region/checkpoint"
        ckpt_dict = {
            "epoch": epoch,
