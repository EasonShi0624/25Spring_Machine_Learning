...
# mean/std of your dataset (ImageNet‐style is a common default)
MEAN = [0.485, 0.456, 0.406]
STD  = [0.229, 0.224, 0.225]

train_transforms = T.Compose([
    # 1. Spatial invariance (tight scale & slight aspect jitter)
    T.RandomResizedCrop(128, scale=(0.85, 1.0), ratio=(0.9, 1.1)),
    T.RandomHorizontalFlip(p=0.5),
    T.RandomRotation(15),

    # 2. Very mild color‐only jitter
    T.RandomApply([
        T.ColorJitter(brightness=0.1, contrast=0.1)
    ], p=0.3),

    # 3. Simulate slight defocus
    T.RandomApply([
        T.GaussianBlur(kernel_size=3, sigma=(0.1, 1.0))
    ], p=0.2),

    # 4. Small random erasing (forces inpainting ability)
...
def train_vae_recon_only(model, train_loader, val_loader, optimizer, device,
    ...
    for epoch in range(1, num_epochs+1):
        ...
        # Save checkpoint
        ckpt_dir="/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v6.05_stronger_augmentaion/checkpoint"
        torch.save(model.state_dict(), f"{ckpt_dir}/stronger_augmentation_epoch{epoch}.pt")
