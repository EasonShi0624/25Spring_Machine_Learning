...
class SpatialSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = (out + seq).permute(0,2,1).view(B,C,H,W)
        return out
...
class ConvVAE(nn.Module):
    def __init__(self, input_channels=3, latent_channels=32, num_classes=170):
        ...
        self.up1 = nn.Sequential(
            nn.ConvTranspose2d(latent_channels, 128, 4, 2, 1),  # 16→32
            nn.BatchNorm2d(128), nn.PReLU())
        ...
        # 64 → 128
        self.up3 = nn.ConvTranspose2d(64, input_channels, 4, 2, 1)  # final Sigmoid later
        self.act_out = nn.Sigmoid()

        # ---- Classifier on flattened latent ----
        self.classifier = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(latent_channels*16*16, num_classes)
        )
    ...
    def encode(self, x):
        x = self.preprocess(x)
        h1 = self.enc1(x)          # 64×64   (save skip)
        h2 = self.enc2(h1)         # 32×32   (save skip)
        h3 = self.enc3(h2)         # 16×16

        h  = self.quant(h3)
        h  = self.attn(h)          # global context
        mean, logvar = torch.chunk(h, 2, dim=1)

        # store skips for decode()
        self._skips = (h1, h2)
    ...
    # ---------- decode ----------
    def decode(self, z):
        h1_skip, h2_skip = self._skips   # 64×64, 32×32

        x = self.up1(z)                  # 32×32
        x = self.fuse1(torch.cat([x, h2_skip], dim=1))

        x = self.up2(x)                  # 64×64
        x = self.fuse2(torch.cat([x, h1_skip], dim=1))
...
def train_vae(model, train_loader, val_loader, optimizer, device,
    ...
    for epoch in range(1, num_epochs+1):
        ...
        ckpt_name = f"/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v4.2unet/checkpoint/checkpoint_beta0.2_epoch{epoch}.pt"
...
if __name__ == "__main__":
    ...
    train_vae(
        ...
        delta=0.8        # perceptual loss weight
    )
