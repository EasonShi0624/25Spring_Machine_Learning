...
class ConvVAE(nn.Module):
    def __init__(self, input_channels=3, latent_channels=32, num_classes=170):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(input_channels,  64, 3, 2, 1),  nn.BatchNorm2d(64),  nn.PReLU(num_parameters=64),
            nn.Conv2d(64,             128, 3, 2, 1),  nn.BatchNorm2d(128), nn.PReLU(num_parameters=128),
            nn.Conv2d(128,            256, 3, 2, 1),  nn.BatchNorm2d(256), nn.PReLU(num_parameters=256),
            nn.Dropout2d(p=0.1),
        )
        ...
        self.decoder = nn.Sequential(
            nn.Conv2d(256,          128, 3, 1, 1),    nn.BatchNorm2d(128), nn.PReLU(num_parameters=128),
            nn.ConvTranspose2d(128,  64, 4, 2, 1),    nn.BatchNorm2d(64),  nn.PReLU(num_parameters=64),
            nn.ConvTranspose2d(64,   32, 4, 2, 1),    nn.BatchNorm2d(32),  nn.PReLU(num_parameters=32),
...
def train_vae(model, train_loader, val_loader, optimizer, device,
    ...
    for epoch in range(1, num_epochs+1):
        ...
        ckpt_name = f"/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v4_PReLU/checkpoint/thousand_checkpoint_PRuLU_epoch{epoch}.pt"
...
if __name__ == "__main__":
    ...
    train_vae(
        ...
        delta=1        # perceptual loss weight
    )
