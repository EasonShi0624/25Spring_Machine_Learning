...
"""
python resblock_VE_cls_only.py --resume /gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v5_SiLU_VE/model_weights_only_740.pt
"""
...
class ResidualBlock(nn.Module):
    """
    A basic 2‐layer residual block with SiLU activations.
    If in_channels != out_channels, applies a 1×1 conv on the skip path.
    """
    def __init__(self, in_channels, out_channels):
        ...
        self.act2  = nn.SiLU()
        # if channel dims differ, project the residual
...
class ConvAE(nn.Module):
    def __init__(self, input_channels=3, latent_channels=8, num_classes=170):
        ...
        # Encoder: 128→32 spatial, with residual blocks after each downsample
        self.encoder = nn.Sequential(
            ...
            nn.Conv2d(128, 256, 3, 1, 1),              # refine 32×32
            nn.BatchNorm2d(256), nn.SiLU(),
            nn.Dropout2d(0.25),
            ResidualBlock(256, 256),
        )

        # Latent projection and back-projection
        self.quant_conv      = nn.Conv2d(256, latent_channels, 1)
        self.post_quant_conv = nn.Conv2d(latent_channels, 256, 1)

        # Decoder: 32→128 spatial, with a residual block at the start
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(64, input_channels, 4, 2, 1),  # 64→128
            nn.Sigmoid(),
        )
...
def train(model, train_loader, val_loader, optimizer, scheduler, device,
    ...
    for epoch in range(1, num_epochs+1):
        ...
        print(f" → VAL recon={val_recon:.6f}, acc={val_acc:.6f}")
        # save best model
        ckpt_dir = "/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v5.1_cls_only_resblock/checkpoint"
        ckpt_dict = {
            #"epoch": epoch,
