...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
#新改动：dropout0.1》0.05; 改成cosineLr
...
def train_vae_recon_only(model, train_loader, val_loader, optimizer, device,
    ...
    for epoch in range(1, num_epochs+1):
        ...
        scheduler.step()
        # Save checkpoint
        ckpt_dir="/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v6_morelayerv4/checkpoint"
        torch.save({'model_state_dict':model.state_dict()}, f"{ckpt_dir}/cosine_continue_checkpoint_epoch{epoch}.pt")
...
if __name__ == "__main__":
    ...
    resume_path = "/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v6_morelayerv4/58_checkpoint_fixed.pt"
    if os.path.isfile(resume_path):
        ckpt = torch.load(resume_path, map_location=device)
        model.load_state_dict(ckpt)
        print(f"Resumed from epoch checkpoint.")
    else:
