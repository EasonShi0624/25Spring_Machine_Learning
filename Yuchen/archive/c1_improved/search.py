...
full_dataset = CustomDataset(transform=None)   # no transform yet
all_indices  = list(range(len(full_dataset)))
train_idx, val_idx = train_test_split(
    all_indices,
    test_size=0.1,        # or whatever your split ratio is
    random_state=42,      # for reproducibility
    shuffle=True
)

# 2. Now create *two* dataset objects, each with its own transforms
train_dataset = CustomDataset(transform=train_transforms)
val_dataset   = CustomDataset(transform=val_transforms)

# 3. Wrap them in Subsets using the precomputed indices
train_subset = Subset(train_dataset, train_idx)
val_subset   = Subset(val_dataset,   val_idx)

# 4. Finally, build loaders
...
# ----------------------------
# 7. Training Loop with Validation‐driven LR
# ----------------------------
def train_vae(model, train_loader, val_loader, optimizer, device,
              num_epochs=30, alpha=0.5, gamma=0.1,
              resume=False, checkpoint_path='checkpoint.pth'):
    # decide start epoch
    if resume:
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        # --- end of train epoch: run validation ---
        model.eval()
        val_stats = validate(model, val_loader, device,
                             kl_weight, alpha, gamma, center_loss)
        # build a single scalar to monitor
        val_loss = ( 1000 * val_stats['recon']
                + kl_weight * val_stats['kl']
        ...
        print(f"[Epoch {epoch}] lr={current_lr:.2e}  val_recon={val_stats['recon']:.6f}  val_cls={val_stats['class']:.6f}  val_center={val_stats['center']:.6f}")

        ckpt_dir = "/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/c1_improved/checkpoint"
