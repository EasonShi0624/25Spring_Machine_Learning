...
train_transforms = transforms.Compose([
    ...
    transforms.ColorJitter(0.2,0.2,0.2,0.05),   # milder
    transforms.ToTensor(),
])
...
class HeadV2(nn.Module):
    """
    Stronger classifier head for an 8-channel latent map (B,8,32,32).

    * depthwise-separable block → lets each channel learn its own patterns
    * Squeeze-Excite (1 % extra params) → inexpensive attention
    * GAP ⨁ GMP concat → keeps both average & “peak” responses
    * two-layer MLP with BN + Mish + Dropout
    """
    def __init__(self, latent_channels: int, num_classes: int):
        super().__init__()

        self.conv_pw1 = nn.Conv2d(latent_channels, 128, 1, bias=False)   # 8→128
        self.bn_pw1   = nn.BatchNorm2d(128)
        self.dw3x3    = nn.Conv2d(128, 128, 3, padding=1, groups=128, bias=False)
        self.bn_dw    = nn.BatchNorm2d(128)
        self.act      = nn.Mish()

        # Squeeze-Excite (128 → 16 → 128)
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(), nn.Linear(128, 16), nn.SiLU(),
            nn.Linear(16, 128), nn.Sigmoid()
        )

        # second point-wise conv to 256 ch
        self.conv_pw2 = nn.Conv2d(128, 256, 1, bias=False)
        self.bn_pw2   = nn.BatchNorm2d(256)

        # MLP head
        self.fc = nn.Sequential(
            nn.Linear(512, 256, bias=False),   # 256 (GAP) ⨁ 256 (GMP) = 512 in
            nn.BatchNorm1d(256),
            nn.Mish(),
            nn.Dropout(0.25),
    ...
    def forward(self, z):                       # z : (B,8,32,32)
        x = self.act(self.bn_pw1(self.conv_pw1(z)))
        x = self.act(self.bn_dw(self.dw3x3(x)))

        # squeeze-excite
        w = self.se(x).view(x.size(0), -1, 1, 1)
        x = x * w

        x = self.act(self.bn_pw2(self.conv_pw2(x)))   # (B,256,32,32)

        # global avg-pool  and  global max-pool
        gap = F.adaptive_avg_pool2d(x, 1).view(x.size(0), -1)   # (B,256)
        gmp = F.adaptive_max_pool2d(x, 1).view(x.size(0), -1)   # (B,256)
        h   = torch.cat([gap, gmp], dim=1)                      # (B,512)
...
class ConvAE(nn.Module):
    def __init__(self, input_channels=3, latent_channels=8, num_classes=170):
        ...
        self.bottleneck_conv      = nn.Conv2d(256, 32 * 2, 1)
        # map latent_channels→256 for decoder
        self.post_bottleneck_conv = nn.Conv2d(32, 256, 1)
        ...
        self.classifier = HeadV2(latent_channels, num_classes)
...
def train_vae(model, train_loader, val_loader, optimizer, device, num_epochs=1, patience=50, alpha=0.1):
    ...
    for epoch in range(1, num_epochs+1):
        ...
        ckpt_dir="/gpfsnyu/scratch/ys6132/25spring_machine_learning/Yuchen/v10089_final_final/checkpoint"
        torch.save(model.state_dict(), f"{ckpt_dir}/10089_epoch{epoch}.pt")
